...
IUSE="debug nls libressl static-libs"

RDEPEND="!libressl? ( dev-libs/openssl:0=[${MULTILIB_USEDEP}] )
...
multilib_src_compile() {
	local ADDCFLAGS=(
		...
		$(use libressl && echo -DOPENSSL_RUNTIME -rdynamic)
	)

	local ADDLDFLAGS=(
		$(use libressl && echo -ldl)
	)
